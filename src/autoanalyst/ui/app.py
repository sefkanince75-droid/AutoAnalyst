...
I18N = {
    "en": {
        "title": "AutoAnalyst V2", "subtitle": "Local, deterministic tabular analytics with reproducible runs.",
        "data": "Data", "analysis": "Analysis", "results": "Results", "history": "History",
        "project": "Project", "dataset": "Dataset", "create_project": "Create project", "create_dataset": "Create dataset",
        "import": "Import", "prepare": "Prepare data", "preview": "Preview", "apply": "Apply preview",
        "run": "Run", "cancel": "Cancel run", "refresh": "Refresh status", "export": "Export report",
        "no_project": "Create a project to begin.", "no_dataset": "Create a dataset to continue.", "no_data": "Import data to continue.",
        "completed": "Completed", "running": "Run in progress", "cancel_requested": "Cancellation requested.",
    },
    "tr": {
        "title": "AutoAnalyst V2", "subtitle": "Yerel, deterministik ve tekrar üretilebilir tablo analizi.",
        "data": "Veri", "analysis": "Analiz", "results": "Sonuçlar", "history": "Geçmiş",
        "project": "Proje", "dataset": "Veri seti", "create_project": "Proje oluştur", "create_dataset": "Veri seti oluştur",
        "import": "İçe aktar", "prepare": "Veriyi hazırla", "preview": "Önizle", "apply": "Önizlemeyi uygula",
        "run": "Çalıştır", "cancel": "Çalışmayı iptal et", "refresh": "Durumu yenile", "export": "Rapor oluştur",
        "no_project": "Başlamak için bir proje oluşturun.", "no_dataset": "Devam etmek için bir veri seti oluşturun.", "no_data": "Devam etmek için veri içe aktarın.",
        "completed": "Tamamlandı", "running": "Çalışma sürüyor", "cancel_requested": "İptal isteği gönderildi.",
    },
}
...
def _select_project(service: ProjectService, language: str):
    ...
    selected = st.sidebar.selectbox(t["project"], projects, index=index, format_func=lambda item: item.name)
...
def _select_dataset(service: DatasetService, project_id: str, language: str):
    ...
    index = ids.index(selected_id) if selected_id in ids else 0
    selected = st.sidebar.selectbox(t["dataset"], datasets, index=index, format_func=lambda item: item.name)
...
def _data_tab(runtime, project, dataset, datasets: DatasetService, language: str) -> None:
    ...
    uploaded = st.file_uploader("CSV / XLSX", type=("csv", "xlsx"), key=f"upload_{dataset.dataset_id}")
    ...
    if version is None:
        st.info(t["no_data"])
        return
    st.success(f"{version.row_count:,} rows · {version.column_count} columns · `{version.version_id[:8]}`")
...
def _preparation_panel(runtime, project_id: str, version_id: str, columns: list[dict[str, object]], language: str) -> None:
    t = I18N[language]
    ...
    operation = st.selectbox("Operation / İşlem", ("filter_rows", "rename_column", "cast_column", "drop_columns", "fill_missing", "drop_missing_rows", "remove_duplicates", "replace_non_finite"))
    params: dict[str, object] = {}
    if operation == "filter_rows":
        name = st.selectbox("Column / Sütun", tuple(by_name), key="prep_filter_col")
        operator = st.selectbox("Operator", ("eq", "ne", "gt", "gte", "lt", "lte", "contains_literal", "is_null", "is_not_null"))
        ...
        params = {"column_id": by_name[name], "target_type": st.selectbox("Type / Tür", ("numeric", "categorical", "boolean", "datetime")), "failure_policy": st.selectbox("Failure policy", ("strict", "invalid_to_null"))}
    elif operation in {"drop_columns", "drop_missing_rows", "remove_duplicates", "replace_non_finite"}:
    ...
    if st.button(t["preview"], key="preview_preparation"):
        try:
            recipe = service.create_recipe(project_id=project_id, base_version_id=version_id, steps=({"operation": operation, "parameters": params},))
    ...
    if preview_id:
        try:
            ...
            st.json({"rows_before": preview.before_row_count, "rows_after": preview.after_row_count, "columns_before": preview.before_column_count, "columns_after": preview.after_column_count, "warnings": list(preview.warnings)})
...
def _analysis_tab(runtime, project, dataset, datasets: DatasetService, language: str) -> None:
    ...
    choices = ("Data profile", "Distribution", "Group summary", "Crosstab", "Correlation", "Welch two groups", "Fisher 2x2", "Binary classification", "Final holdout", "Score new data")
    task = st.selectbox("Analysis / Analiz", choices)
    try:
        if task == "Data profile":
            if st.button(t["run"], key="run_profile"):
                _launch(runtime, _spec(project.project_id, version.version_id, AnalysisModuleId.PROFILING, "profile", {}, ()))
        elif task == "Distribution":
            name = st.selectbox("Column / Sütun", tuple(by_name), key="dist_col")
            if st.button(t["run"], key="run_distribution"):
                _launch(runtime, _spec(project.project_id, version.version_id, AnalysisModuleId.COMPARISON, "distribution", {"column_id": str(by_name[name]["column_id"])}, ()))
        elif task == "Group summary":
            groups = st.multiselect("Group columns", tuple(by_name), max_selections=2, key="summary_groups")
            measures = st.multiselect("Measures", tuple(by_name), max_selections=5, key="summary_measures")
            if st.button(t["run"], key="run_group_summary"):
                params = {"group_columns": [str(by_name[n]["column_id"]) for n in groups], "measure_columns": [str(by_name[n]["column_id"]) for n in measures]}
                _launch(runtime, _spec(project.project_id, version.version_id, AnalysisModuleId.COMPARISON, "group_summary", params, ()))
        elif task == "Crosstab":
            row_name = st.selectbox("Row", tuple(by_name), key="xtab_row")
            col_name = st.selectbox("Column", tuple(by_name), key="xtab_col", index=min(1, len(by_name)-1))
            if st.button(t["run"], key="run_crosstab"):
                params = {"row_column_id": str(by_name[row_name]["column_id"]), "column_column_id": str(by_name[col_name]["column_id"])}
                _launch(runtime, _spec(project.project_id, version.version_id, AnalysisModuleId.COMPARISON, "crosstab", params, ()))
            ...
            if st.button(t["run"], key="run_corr"):
                ...
                _launch(runtime, _spec(project.project_id, version.version_id, AnalysisModuleId.COMPARISON, "correlation", params, ()))
...
def _welch_form(runtime, project_id: str, version_id: str, by_name: dict[str, dict[str, object]], t: dict[str, str]) -> None:
    ...
    group_b = st.selectbox("Group B", values, key="welch_b", index=min(1, len(values)-1), format_func=repr)
    if st.button(t["run"], key="run_welch"):
        params = {"group_column_id": str(by_name[group_name]["column_id"]), "measure_column_id": str(by_name[measure_name]["column_id"]), "group_a": group_a, "group_b": group_b}
        _launch(runtime, _spec(project_id, version_id, AnalysisModuleId.COMPARISON, "welch_two_groups", params, ()))


def _fisher_form(runtime, project_id: str, version_id: str, by_name: dict[str, dict[str, object]], t: dict[str, str]) -> None:
    row_name = st.selectbox("Row variable", tuple(by_name), key="fisher_row")
    col_name = st.selectbox("Column variable", tuple(by_name), key="fisher_col", index=min(1, len(by_name)-1))
    ...
    if st.button(t["run"], key="run_fisher"):
        params = {"row_column_id": str(by_name[row_name]["column_id"]), "column_column_id": str(by_name[col_name]["column_id"]), "row_positive": row_positive, "column_positive": col_positive}
        _launch(runtime, _spec(project_id, version_id, AnalysisModuleId.COMPARISON, "fisher_2x2", params, ()))


def _binary_form(runtime, project_id: str, version_id: str, by_name: dict[str, dict[str, object]], t: dict[str, str]) -> None:
    target_name = st.selectbox("Target", tuple(by_name), key="binary_target")
    feature_names = st.multiselect("Features", tuple(name for name in by_name if name != target_name), key="binary_features")
    target_id = str(by_name[target_name]["column_id"])
    labels = _distinct_values(runtime, version_id, by_name[target_name])
    positive = st.selectbox("Positive event / Pozitif olay", labels, format_func=repr) if labels else None
    split = st.selectbox("Split", ("stratified", "group", "time"))
    group_name = st.selectbox("Group column", tuple(by_name), key="binary_group") if split == "group" else None
    time_name = st.selectbox("Time column", tuple(by_name), key="binary_time") if split == "time" else None
    ...
    if st.button(t["run"], key="run_binary"):
        roles: list[ColumnRole] = [ColumnRole(str(by_name[name]["column_id"]), _semantic(by_name[name]), (ColumnUsage.FEATURE,), True, True) for name in feature_names]
        roles.append(ColumnRole(target_id, _semantic(by_name[target_name]), (ColumnUsage.TARGET,), True))
        if group_name:
            roles.append(ColumnRole(str(by_name[group_name]["column_id"]), _semantic(by_name[group_name]), (ColumnUsage.GROUP,), True))
        if time_name:
            roles.append(ColumnRole(str(by_name[time_name]["column_id"]), _semantic(by_name[time_name]), (ColumnUsage.TIME,), True))
        params = {"positive_label": positive, "minimum_recall": minimum_recall, "split_policy": split, "class_weight_policy": class_weight}
        _launch(runtime, _spec(project_id, version_id, AnalysisModuleId.BINARY_CLASSIFICATION, "train_validate", params, tuple(roles)))
...
def _final_form(runtime, project_id: str, t: dict[str, str]) -> None:
    ...
    if not candidates:
        ...
        return
    selected = st.selectbox("Training run", candidates, format_func=lambda item: f"{item[0].created_at.isoformat()} · {item[2].provenance['recommended_model']['model_id']}")
    run, training_spec, _ = selected
    lock = __import__("autoanalyst.storage.binary", fromlist=["BinaryStore"]).BinaryStore(runtime.catalog).get_holdout_lock(run.run_id)
    if lock:
        st.info(f"Holdout: {lock['status']}")
    if st.button("Open final holdout / Son testi aç", key="open_holdout", disabled=bool(lock and lock["status"] == "reported")):
        spec = _spec(project_id, training_spec.input_version_id, AnalysisModuleId.BINARY_CLASSIFICATION, "final_evaluate", {"training_run_id": run.run_id}, training_spec.column_roles)
        _launch(runtime, spec, parent_run_id=run.run_id)


def _score_form(runtime, project_id: str, version_id: str, by_name: dict[str, dict[str, object]], t: dict[str, str]) -> None:
    candidates = _training_candidates(runtime, project_id)
    if not candidates:
        st.info("Train a model first / Önce model eğitin.")
        return
    selected = st.selectbox("Model run", candidates, format_func=lambda item: f"{item[0].created_at.isoformat()} · {item[2].provenance['recommended_model']['model_id']}")
    run, training_spec, result = selected
    training_columns = {str(row["column_id"]): row for row in runtime.catalog.list_columns(training_spec.input_version_id)}
    ...
    for training_id in result.provenance["feature_column_ids"]:
        ...
        selected_name = st.selectbox(f"{name} →", choices, index=default, key=f"score_map_{training_id}")
        mapping[training_id] = str(by_name[selected_name]["column_id"])
    if st.button("Score / Tahmin et", key="run_scoring"):
        spec = _spec(project_id, version_id, AnalysisModuleId.BINARY_CLASSIFICATION, "score_new_data", {"training_run_id": run.run_id, "feature_mapping": mapping}, ())
        _launch(runtime, spec, parent_run_id=run.run_id)


def _results_tab(runtime, project, language: str) -> None:
    t = I18N[language]
    runs = [run for run in runtime.run_store.list_runs(project.project_id) if run.status is RunStatus.COMPLETED and run.result_id]
    if not runs:
        st.info("No completed results / Tamamlanmış sonuç yok.")
        return
    default_id = st.session_state.get("last_result_id")
    default_index = next((i for i, run in enumerate(runs) if run.result_id == default_id), len(runs)-1)
    run = st.selectbox("Run / Çalışma", runs, index=default_index, format_func=lambda item: f"{item.created_at.isoformat()} · {runtime.run_store.get_spec(item.spec_id).operation if item.spec_id else item.kind.value}")
    result = runtime.run_store.get_result(run.result_id)
    st.write(f"**{result.module_id.value}** · `{result.outcome.value}`")
    if result.metrics:
        st.dataframe(pd.DataFrame([{"metric": metric.name, "value": metric.value if metric.value is not None else metric.value_state.value, **dict(metric.dimensions)} for metric in result.metrics]), use_container_width=True, hide_index=True)
    for finding in result.findings:
        (st.info if finding.severity.value == "info" else st.warning)(f"{finding.code}: {dict(finding.parameters)}")
    ...
    report_language = st.selectbox("Report language / Rapor dili", ("en", "tr"), index=0 if language == "en" else 1)
    ...
    if report_id := st.session_state.get("report_id"):
        try:
            ...
            extension = "zip" if report.format == "csv" else "md" if report.format == "markdown" else report.format
            st.download_button("Download / İndir", service.artifact_bytes(report), file_name=f"autoanalyst-{result.result_id[:8]}.{extension}")
        except Exception:
...
def _history_tab(runtime, project, dataset, datasets: DatasetService) -> None:
    ...
    st.dataframe(pd.DataFrame([{"run_id": run.run_id, "status": run.status.value, "created_at": run.created_at, "result_id": run.result_id, "error_code": run.error_code} for run in runs]), use_container_width=True, hide_index=True)
...
def _distinct_values(runtime, version_id: str, column: dict[str, object]) -> list[object]:
    ...
    frame = TableAccess(runtime.catalog, runtime.artifact_store).selected_columns(version_id, (column_id,)).to_pandas()
