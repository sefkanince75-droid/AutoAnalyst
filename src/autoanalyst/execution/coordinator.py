...
class ExecutionCoordinator:
    ...
    def execute_inline(
        ...
        *,
        ...
        if issues:
            raise UnexpectedExecutionError(
                {"reason": "analysis_spec_validation_failed", "codes": tuple(item.code for item in issues)}
            )
        applicability = module.check_applicability(spec)
        ...
        try:
            ...
            self.store.append_event(
                run_id, EventKind.FAILED.value, stage="run", payload={"error_code": exc.code.value}
            )
            raise
            ...
            self.store.append_event(
                ...
                stage="run",
                payload={"error_code": "unexpected_execution", "exception_type": type(exc).__name__},
            )
            raise UnexpectedExecutionError(
