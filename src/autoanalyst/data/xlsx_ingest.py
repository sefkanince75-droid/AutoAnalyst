...
class XLSXIngestor:
    ...
    def sheet_names(self, source: bytes | bytearray | str | Path) -> tuple[str, ...]:
        ...
        try:
            ...
            raise DataError({"reason": "xlsx_open_failed", "exception_type": type(exc).__name__}) from exc

    def import_xlsx(self, *, project_id: str, dataset_id: str, source: bytes | bytearray | str | Path, sheet_name: str, original_name: str | None = None) -> IngestionResult:
        ...
        try:
            ...
            raise DataError({"reason": "xlsx_parse_failed", "exception_type": type(exc).__name__}) from exc
        ...
        raw = self.artifact_store.finalize(self.artifact_store.stage_bytes(raw_bytes, project_id=project_id, owner_run_id=import_id, kind="raw_source", media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"))
        ...
        canonical = dataframe.rename(columns={column.display_name: column.physical_name for column in user_columns}).copy()
        canonical.insert(0, columns[0].physical_name, [str(uuid5(NAMESPACE_URL, f"autoanalyst:{raw_sha256}:{sheet_name}:{index}")) for index in range(len(canonical))])
    ...
    def _write_parquet(self, dataframe: pd.DataFrame, project_id: str, import_id: str) -> Artifact:
        ...
        staged = self.artifact_store.create_staging(project_id=project_id, owner_run_id=import_id, kind="canonical_table", media_type="application/vnd.apache.parquet", format_version="1")
        try:
            pq.write_table(pa.Table.from_pandas(dataframe, preserve_index=False), staged.staging_path, compression="zstd")
