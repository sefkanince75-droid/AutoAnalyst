...
class ComparisonModule:
    ...
    def estimate_resources(self, spec: AnalysisSpec) -> ResourceEstimate:
        ...
        return ResourceEstimate(
            max(8_000_000, cells * 24),
            max(1_000_000, cells * 4),
            max(0.1, cells / 3_000_000),
            1,
        )
    ...
    def run(self, spec: AnalysisSpec, context: ExecutionContext) -> ResultDraft:
        ...
        if issues:
            raise SchemaError(
                {"reason": "invalid_comparison_spec", "codes": tuple(i.code for i in issues)}
            )
    ...
    def _group_summary(self, spec: AnalysisSpec) -> ResultDraft:
        ...
        for keys, subset in frame.groupby(list(groups), dropna=False, sort=True):
            ...
            for measure in measures:
                numeric = pd.to_numeric(subset[measure], errors="coerce").replace(
        ...
        table = ResultTable(
            ...
            tuple(
                ...
                + ["measure_column_id", "n", "missing", "mean", "median", "std", "min", "max"]
            ),
            tuple(rows),
        )
    ...
    def _distribution(self, spec: AnalysisSpec) -> ResultDraft:
        ...
        if len(numeric_valid) >= max(1, int(series.notna().sum() * 0.8)):
            counts, edges = np.histogram(
                numeric_valid.to_numpy(dtype=float), bins=int(spec.parameters.get("bins", 20))
            )
            table = ResultTable(
                str(uuid4()),
            ...
            counts = (
                ...
                .head(int(spec.parameters.get("top_k", 20)))
            )
            table = ResultTable(
                str(uuid4()),
    ...
    def _correlation(self, spec: AnalysisSpec) -> ResultDraft:
        ...
        metric = _metric(
            "correlation",
            value,
            {"method": method, "x_column_id": x_id, "y_column_id": y_id},
            len(valid),
        )
        return self._draft(
            spec,
            metrics=(metric,),
            sample={"rows_used": len(valid), "rows_excluded": len(frame) - len(valid)},
        )

    def _welch(self, spec: AnalysisSpec) -> ResultDraft:
        group_id = str(spec.parameters["group_column_id"])
        measure_id = str(spec.parameters["measure_column_id"])
        a_label, b_label = spec.parameters["group_a"], spec.parameters["group_b"]
        frame, _ = self._load(spec, (group_id, measure_id))
        values = pd.to_numeric(frame[measure_id], errors="coerce").replace(
            [np.inf, -np.inf], np.nan
        )
        ...
        if len(a) < 30 or len(b) < 30:
            findings = (
                Finding(
                    str(uuid4()),
                    "comparison.small_group_sample",
                    FindingSeverity.WARNING,
                    parameters={"n_a": len(a), "n_b": len(b)},
                ),
            )
        return self._draft(
            spec,
            metrics=metrics,
            findings=findings,
            sample={"rows_used": len(a) + len(b), "rows_excluded": len(frame) - len(a) - len(b)},
        )

    def _fisher(self, spec: AnalysisSpec) -> ResultDraft:
        row_id = str(spec.parameters["row_column_id"])
        col_id = str(spec.parameters["column_column_id"])
        row_positive, col_positive = (
            spec.parameters["row_positive"],
            spec.parameters["column_positive"],
        )
        frame, _ = self._load(spec, (row_id, col_id))
        used = frame.dropna(subset=[row_id, col_id])
        row_values = list(used[row_id].drop_duplicates())
        col_values = list(used[col_id].drop_duplicates())
        if (
            len(row_values) != 2
            or len(col_values) != 2
            or row_positive not in row_values
            or col_positive not in col_values
        ):
        ...
        table = ResultTable(
            ...
            ("row", "column", "count"),
            (
                ("positive", "positive", int(matrix[0, 0])),
                ("positive", "negative", int(matrix[0, 1])),
                ("negative", "positive", int(matrix[1, 0])),
                ("negative", "negative", int(matrix[1, 1])),
            ),
        )
        return self._draft(
            spec,
            metrics=(odds_metric, p_metric),
            tables=(table,),
            sample={"rows_used": len(used), "rows_excluded": len(frame) - len(used)},
        )
    ...
    def _draft(
        ...
        if spec.operation == "welch_two_groups":
            methodology["independence_confirmed"] = True
        return ResultDraft(
