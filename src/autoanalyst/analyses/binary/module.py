...
class BinaryClassificationModule:
    ...
    def describe(self) -> AnalysisDescription:
        return AnalysisDescription(AnalysisModuleId.BINARY_CLASSIFICATION, self.MODULE_VERSION, self.OPERATIONS)
    ...
    def validate(self, spec: AnalysisSpec) -> tuple[ValidationIssue, ...]:
        ...
        if spec.operation == "train_validate":
            ...
            if isinstance(minimum_recall, bool) or not isinstance(minimum_recall, (int, float)) or not 0 < float(minimum_recall) <= 1:
                issues.append(ValidationIssue("binary.invalid_minimum_recall"))
        if spec.operation in {"final_evaluate", "score_new_data"} and "training_run_id" not in spec.parameters:
    ...
    def check_applicability(self, spec: AnalysisSpec) -> ApplicabilityReport:
        ...
        if spec.operation == "train_validate" and version.row_count < 40:
            return ApplicabilityReport(blocking_issues=(ValidationIssue("binary.dataset_too_small"),))
    ...
    def estimate_resources(self, spec: AnalysisSpec) -> ResourceEstimate:
        ...
        return ResourceEstimate(max(16_000_000, min(4_000_000_000, cells * multiplier)), max(4_000_000, min(1_000_000_000, cells * 12)), max(0.2, cells / 750_000), 1)

    def run(self, spec: AnalysisSpec, context: ExecutionContext) -> ResultDraft:
        issues = self.validate(spec)
        if issues:
            raise SchemaError({"reason": "invalid_binary_spec", "codes": tuple(i.code for i in issues)})
        if spec.operation == "train_validate":
    ...
    def _train_validate(self, spec: AnalysisSpec, context: ExecutionContext) -> ResultDraft:
        ...
        unsupported = [role.column_id for role in feature_roles if role.semantic_type in {SemanticType.DATETIME, SemanticType.IDENTIFIER, SemanticType.TEXT}]
        if unsupported:
            raise MethodNotApplicableError({"reason": "binary.unsupported_feature_type", "column_ids": tuple(unsupported)})

        system_row = next(row for row in self.catalog.list_columns(spec.input_version_id) if row["physical_name"] == INTERNAL_ROW_ID)
        ...
        if len(labels) != 2:
            raise MethodNotApplicableError({"reason": "binary.target_not_binary", "distinct": len(labels)})
        ...
        numeric = [role.column_id for role in feature_roles if role.semantic_type is SemanticType.NUMERIC]
        categorical = [role.column_id for role in feature_roles if role.semantic_type in {SemanticType.CATEGORICAL, SemanticType.BOOLEAN}]
        X = _prepare_features(work[feature_ids], numeric, categorical)
        if numeric and not np.isfinite(X[numeric].to_numpy(dtype=float, na_value=np.nan)[~pd.isna(X[numeric]).to_numpy()]).all():
        ...
        partitions = make_partitions(split_frame, target="__aa_target_binary__", strategy=split_policy, seed=spec.seed, group=group_id, time=time_id)

        class_weight = str(spec.parameters.get("class_weight_policy", "none"))
        models = build_models(numeric, categorical, seed=spec.seed, class_weight_policy=class_weight)
        ...
        for model_id, model in models.items():
            ...
            with warnings.catch_warnings(record=True) as caught:
                ...
                if any(issubclass(item.category, ConvergenceWarning) for item in caught):
                    raise MethodNotApplicableError({"reason": "binary.model_convergence_failure", "model_id": model_id})
            scores = positive_scores(model, X.iloc[partitions.validation], 1)
            selected = select_threshold(y.iloc[partitions.validation].to_numpy(), scores, positive_label=1, minimum_recall=minimum_recall)
            if selected is None:
                summaries.append({"model_id": model_id, "feasible": False})
                fitted[model_id] = model
                continue
            metrics = evaluate_scores(y.iloc[partitions.validation].to_numpy(), scores, positive_label=1, negative_label=0, threshold=selected.threshold)
            summaries.append({"model_id": model_id, "feasible": True, "threshold": selected.threshold, "precision": metrics.precision, "recall": metrics.recall, "average_precision": metrics.average_precision, "roc_auc": metrics.roc_auc, "f1": metrics.f1})
            fitted[model_id] = model
        ...
        if recommendation is not None:
            model_artifact = self._save_model(fitted[str(recommendation["model_id"])], spec, context)
            artifacts.append(model_artifact)
        split_artifact = self._save_split(frame[str(system_row["column_id"])], partitions, spec, context)
        artifacts.append(split_artifact)

        rows = tuple((item.get("model_id"), bool(item.get("feasible")), item.get("threshold"), item.get("precision"), item.get("recall"), item.get("average_precision"), item.get("roc_auc"), item.get("f1")) for item in summaries)
        table = ResultTable(str(uuid4()), ("model_id", "feasible", "threshold", "precision", "recall", "average_precision", "roc_auc", "f1"), rows)
        metrics: list[Metric] = []
        for item in summaries:
            if not item.get("feasible"):
                continue
            for name in ("precision", "recall", "average_precision", "roc_auc", "f1"):
                metrics.append(_metric(name, float(item[name]), {"model_id": str(item["model_id"]), "partition": "validation"}))
        findings: tuple[Finding, ...] = ()
        outcome = ResultOutcome.SUCCEEDED
        if recommendation is None:
            outcome = ResultOutcome.PARTIAL
            findings = (Finding(str(uuid4()), "binary.no_baseline_improvement", FindingSeverity.WARNING),)
        provenance = {
            ...
            "feature_semantic_types": {role.column_id: role.semantic_type.value for role in feature_roles},
        ...
        return ResultDraft(outcome=outcome, metrics=tuple(metrics), findings=findings, tables=(table,), artifacts=tuple(artifacts), methodology={"module": "binary_classification", "module_version": self.MODULE_VERSION, "operation": "train_validate", "models": ("dummy", "logistic_regression", "random_forest"), "minimum_recall": minimum_recall}, sample_summary={"train": len(partitions.train), "validation": len(partitions.validation), "test_locked": len(partitions.test)}, provenance=provenance)
    ...
    def _final_evaluate(self, spec: AnalysisSpec, context: ExecutionContext) -> ResultDraft:
        ...
        selection_hash = fingerprint({"training_run_id": training_run_id, "model_sha": model_meta["sha256"], "split_sha": split_meta["sha256"], "threshold": recommendation["threshold"]})
        ...
        y = test[target_id].map(lambda value: 1 if _typed_key(_py(value)) == _typed_key(positive) else 0)
        semantic = dict(provenance["feature_semantic_types"])
        numeric = [column for column in feature_ids if semantic.get(column) == "numeric"]
        categorical = [column for column in feature_ids if semantic.get(column) in {"categorical", "boolean"}]
        ...
        evaluated = evaluate_scores(y.to_numpy(), scores, positive_label=1, negative_label=0, threshold=threshold)
        metrics = tuple(_metric(name, float(getattr(evaluated, name)), {"partition": "final_test", "model_id": recommendation["model_id"]}) for name in ("precision", "recall", "average_precision", "roc_auc", "f1"))
        return ResultDraft(outcome=ResultOutcome.SUCCEEDED, metrics=metrics, methodology={"module": "binary_classification", "operation": "final_evaluate", "holdout_access": "locked_before_read"}, sample_summary={"test": len(test)}, provenance={"training_run_id": training_run_id, "selection_hash": selection_hash, "threshold": threshold, "model_id": recommendation["model_id"]})
    ...
    def _score(self, spec: AnalysisSpec, context: ExecutionContext) -> ResultDraft:
        ...
        remapped = pd.DataFrame({training: frame[scoring] for training, scoring in zip(training_features, scoring_ids, strict=True)})
        semantic = dict(provenance["feature_semantic_types"])
        numeric = [column for column in training_features if semantic.get(column) == "numeric"]
        categorical = [column for column in training_features if semantic.get(column) in {"categorical", "boolean"}]
        ...
        table = ResultTable(str(uuid4()), ("row_id", "score_positive", "predicted_label"), tuple((str(row_id), float(score), _py(label)) for row_id, score, label in zip(frame[row_id_col], scores, labels, strict=True)))
        return ResultDraft(outcome=ResultOutcome.SUCCEEDED, tables=(table,), methodology={"module": "binary_classification", "operation": "score_new_data", "threshold_locked": True, "refit": False}, sample_summary={"rows_scored": len(frame)}, provenance={"training_run_id": training_run_id, "model_id": recommendation["model_id"], "threshold": threshold})
    ...
    def _load_as_ids(self, version_id: str, column_ids: tuple[str, ...]) -> pd.DataFrame:
        ...
        return frame.rename(columns={str(metadata[column]["physical_name"]): column for column in column_ids})
        ...
        while stack:
            ...
            if version.recipe_id:
                ...
                for step in recipe.ordered_steps:
                    if step.learning_scope is LearningScope.DATASET and protected.intersection(step.affected_column_ids):
                        raise MethodNotApplicableError({"reason": "binary.dataset_level_leakage", "step_id": step.step_id, "operation": step.operation})
    ...
    def _save_model(self, model, spec: AnalysisSpec, context: ExecutionContext) -> Artifact:
        staged = self.artifact_store.create_staging(project_id=spec.project_id, owner_run_id=context.run_id, kind="binary_model", media_type="application/x-joblib", format_version="2.0")
        joblib.dump(model, staged.staging_path)
        return self.artifact_store.finalize(staged)

    def _save_split(self, row_ids: pd.Series, partitions, spec: AnalysisSpec, context: ExecutionContext) -> Artifact:
        ...
        staged = self.artifact_store.create_staging(project_id=spec.project_id, owner_run_id=context.run_id, kind="split_manifest", media_type="application/vnd.apache.parquet", format_version="2.0")
...
def _prepare_features(frame: pd.DataFrame, numeric: list[str], categorical: list[str]) -> pd.DataFrame:
    ...
    for column in categorical:
        result[column] = result[column].map(lambda value: np.nan if pd.isna(value) else canonical_json(encode_typed_label(_py(value))))
    return result
...
def _recommend(items: list[dict[str, object]]) -> dict[str, object] | None:
    ...
    candidates = [feasible[name] for name in ("logistic_regression", "random_forest") if name in feasible]
    if not candidates or dummy is None:
        return None
    best = sorted(candidates, key=lambda item: (float(item["precision"]), float(item["average_precision"]), float(item["recall"]), 1 if item["model_id"] == "logistic_regression" else 0), reverse=True)[0]
    if float(best["precision"]) <= float(dummy["precision"]) + 1e-12 and float(best["average_precision"]) <= float(dummy["average_precision"]) + 1e-12:
        return None
    return {"model_id": best["model_id"], "threshold": best["threshold"], "precision": best["precision"], "recall": best["recall"], "average_precision": best["average_precision"]}
...
def _artifact_payload(artifact: Artifact | None):
    if artifact is None:
        return None
    return {"artifact_id": artifact.artifact_id, "relative_path": artifact.relative_path, "sha256": artifact.sha256, "byte_size": artifact.byte_size, "media_type": artifact.media_type, "kind": artifact.kind}
