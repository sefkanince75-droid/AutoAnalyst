...
def test_xlsx_explicit_sheet_and_offline_reports(tmp_path):
    ...
    spec = AnalysisSpec(
        ...
        spec_hash=fingerprint({"version": imported.version.version_id, "op": "profile"}),
        created_at=utc_now(),
    )
    ...
    coordinator.execute_inline(
        run.run_id, ProfilingModule(catalog, store), cancellation_event=_Event()
    )
    ...
    assert (
        reports.render(result.result_id, format="html", language="en").report_id == html.report_id
    )
